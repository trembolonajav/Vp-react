...
export function AdminWhatsAppPanel() {
  ...
  const action = async <T,>(fn: () => Promise<T>, ok: string) => { setBusy(true); setMessage(""); try { const next = await fn(); if (next && typeof next === "object" && "status" in next) setStatus(next as unknown as WhatsAppStatus); else await refresh(); setMessage(ok); } catch (err) { setMessage((err as Error).message); await refresh(); } finally { setBusy(false); } };
  return <section className="admin-section admin-whatsapp">
    <div className="admin-section-head"><div><span className="admin-intermediary-kicker">Integração interna</span><h2 className="an-block-title">WhatsApp conectado</h2></div><span className={`wa-state wa-status-${status?.status ?? "offline"}`}><i/>{LABEL[status?.status ?? ""] ?? "Indisponível"}</span></div>
    <div className="wa-grid"><div className="wa-connect-card">
      ...
      {status?.status === "connected" ? <button disabled={busy} onClick={() => void action(disconnectWhatsApp,"WhatsApp desconectado e sessão local removida.")}>{busy ? "Desconectando..." : "Desconectar"}</button> : <button disabled={busy || status?.status === "connecting"} onClick={() => void action(connectWhatsApp,"QR Code gerado. Escaneie com o celular.")}>{busy || status?.status === "connecting" ? "Gerando QR Code..." : "Gerar novo QR Code"}</button>}
